...
function ping_or_fail() {
  if ! sudo ip netns exec $1 ping -i 0.1 -c 3 $2; then
...
function check_connectivity() {
  ...
  ping -i 0.1 -c 3 10.0.0.10 || { echo "Ping from Local Machine to Load Balancer failed!"; exit 1; }
...
check_interfaces () {
  for ns in h2 h3 lb; do
    echo ""
    echo "$ns namespace dump"
    sudo ip netns exec $ns ip addr show
  done

  echo ""
  echo "br0 bridge dump..."
  ip addr show br0
  ip link show br0
  echo ""
}
